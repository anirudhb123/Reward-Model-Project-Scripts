...
"""
    ...
    for idx, ex in enumerate(tqdm.tqdm(dataset)):
        ...
        asked_for_hedging  = parts[0] if len(parts) > 0 else ""
        chosen_hedging     = parts[1] if len(parts) > 1 else ""
        rejected_hedging   = parts[2] if len(parts) > 2 else ""

        # Only append into our two buckets, up to the new-target
        # Only append into our two buckets, up to the new-target
        chosen_lower = chosen_hedging.lower()
        rejected_lower = rejected_hedging.lower()
        ...
        record = {
            ...
            "query_classification": asked_for_hedging,
            "chosen_hedging": norm_chosen,
            "rejected_hedging": norm_rejected
        }
