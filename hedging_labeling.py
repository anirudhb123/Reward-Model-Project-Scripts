...
def main(unused_argv) -> None:
    classification_prompt = """ You are a query classifier. Your task is to classify the following query and responses into three categories:
...
2. **Chosen Response Contains Hedging**: Determine if the chosen response includes language that expresses uncertainty, mitigates its claims, or considers multiple perspectives without committing to one.

3. **Rejected Response Contains Hedging**: Determine if the rejected response includes language that expresses uncertainty, mitigates its claims, or considers multiple perspectives without committing to one.
